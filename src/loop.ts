...
export async function processIssue(issue: linear.Issue): Promise<void> {
  ...
  try {
    ...
    // untrusted()-wrapped; a leg with no structured findings (unresolved or
    // empty) falls back to its labeled raw text so nothing is silently dropped.
    ...
    const fixerWritableScope = buildFixerWritableScope(implementerTouchedFiles, findingsFiles(reviewFindings));
    const fixerWriteGuardHooks = guardBypass ? undefined : buildWriteGuardHooks(ws.dir, { writableScope: fixerWritableScope });
    const fixPrompt = ownerFeedbackBlock + renderPrompt(fixerRoute.card, { spec: specForFixer, reviews: untrusted(reviewsForFixer) },
        `You are the fixer in an automated pipeline. Two independent reviewers examined the latest change in this worktree against the ticket; their findings are grouped below BY FILE, most severe (blocker) first. Evaluate each finding, fix the real ones, reject ones that contradict the ticket. Edit ONLY the files named in the findings below — do not restyle or refactor anything beyond what a finding names, even if you notice something else you'd change; your write access this round is scoped to exactly those files plus what the implementer already touched. Never weaken or delete tests. Sanity-check with the repo's own scripts. Reply with one line per finding: fixed / rejected (why).\n\n${specForFixer}\n\n${untrusted(reviewsForFixer)}`);
    ...
    if (uiFilesTouched(ws).length > 0 && !budget.expired) {
      ...
      for (let round = 1; round < maxTasteRounds && tasteFixRoundWarranted(designGate) && !budget.expired; round++) {
        ...
        // directly in this prompt string.
        ...
        const designWritableScope = buildFixerWritableScope(implementerTouchedFiles, findingsFiles(collectFindings(designLegs)));
        const designFixWriteGuardHooks = guardBypass ? undefined : buildWriteGuardHooks(ws.dir, { writableScope: designWritableScope });
