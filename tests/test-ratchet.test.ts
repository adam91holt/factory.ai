...
describe("parsePassingTestCount — tolerant across runner formats, UNKNOWN (null) when unparseable", () => {
  test("bun test summary", async () => {
    const out = "bun test v1.1.0\n\ntests/foo.test.ts:\n✓ a thing works\n\n 631 pass\n 0 fail\n 1310 expect() calls\nRan 631 tests across 33 files. [1.2s]\n";
    expect(parsePassingTestCount(out)).toBe(631);
  });

  test("bun with ANSI color codes (real terminal output)", async () => {
    const out = "\x1b[32m 631 pass\x1b[0m\n\x1b[31m 2 fail\x1b[0m\n";
    expect(parsePassingTestCount(out)).toBe(631);
  });

  test("bun zero-test run parses as 0, not UNKNOWN", async () => {
    expect(parsePassingTestCount(" 0 pass\n 0 fail\n")).toBe(0);
  });

  test("jest/npm summary — the passed number, not failed/total, and not Test Suites", async () => {
...
describe("testCountRatchet — decrease withholds, increase flows, UNKNOWN never blocks", () => {
  ...
  test("a confirmed decrease on one gate WINS over unknown on another (tighten-only)", async () => {
  ...
  test("no test gates at all (strength none / weak-only repo) → skipped", async () => {
  ...
  test("a test gate no-gated by a red baseline (passed null) does not participate", async () => {
  ...
  test("non-test gates never contribute counts even if somehow populated", async () => {
    expect(testCountRatchet([gate("typecheck", true, 9, 1)]).verdict).toBe("skipped");
  });

  test("multi-gate evidence names each gate", async () => {
...
describe("report surfaces the ratchet evidence (queryable 'tests: a -> b')", () => {
  ...
  test("a DECREASED verdict renders the counts, the withhold note, and YAML meta", async () => {
  ...
  test("an unknown verdict is visible but explicitly non-blocking", async () => {
  ...
  test("no testRatchet input (skipped) → no test-count lines at all", async () => {
...
describe("baseline/verify carry test counts end-to-end", () => {
  ...
  test("baseline records the count; verify compares it against the post-change run", async () => {
    const ws = scratch("echo ' 5 pass' && echo ' 0 fail'");
    try {
      const baselines = await baseline(ws, ["test"]);
      ...
      const results = await verify(ws, ["test"], baselines);
  ...
  test("a red baseline records no count and verify no-gates it (existing park logic owns it)", async () => {
