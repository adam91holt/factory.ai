...
describe("gateSummary", () => {
  ...
  test("failures are collected and green flips", async () => {
  ...
  test("build/lint gates are weak, never real", async () => {
...
describe("detectGates", () => {
  test("a `test` script is detected and gateSummary reports the resulting gate as real", async () => {
  ...
  test("test:ci / test:unit variants are also detected", async () => {
  ...
  test("a placeholder `npm init` test script is excluded, not counted as a gate", async () => {
  ...
  test("no package.json → no gates detected", async () => {
    const dir = mkdtempSync(join(tmpdir(), "factory-detectgates-"));
    try {
  ...
  test("browser/e2e scripts are detected as gates (Gap 2)", async () => {
...
describe("isE2eGate", () => {
  test("classifies the browser/e2e script names, exactly", async () => {
    for (const n of ["e2e", "test:e2e", "test:browser", "playwright"]) expect(isE2eGate(n)).toBe(true);
  });
  test("unit-test and unrelated gates are NOT e2e", async () => {
...
describe("gateSummary strength ladder (Gap 2)", () => {
  test("a unit test gate alone is real, not strong", async () => {
    const s = gateSummary([gate("test", true)]);
    expect(s.strength).toBe("real");
    expect(s.hasE2eGate).toBe(false);
  });

  test("unit test + a passing e2e gate is strong", async () => {
  ...
  test("an e2e gate alone (no unit test) is real — a real test that drove the app", async () => {
  ...
  test("a FAILING e2e flips green but strength still reflects capability (strong)", async () => {
  ...
  test("e2e gate + only weak gates (no unit) stays real, never strong", async () => {
...
describe("hasUiSurface", () => {
  test("index.html → UI surface", async () => {
...
describe("runWithRetryOnError (#12a: baseline gate that errors/times out is retried once)", () => {
  ...
  test("an errored/timed-out first attempt is retried once, and the retry's verdict wins", async () => {
    let calls = 0;
    const r = await runWithRetryOnError(() => { calls++; return calls === 1 ? errored() : pass(); });
    expect(calls).toBe(2);
    expect(r).toEqual(pass());
  });

  test("a CLEAN non-zero exit (genuinely red baseline) is NOT retried", async () => {
    let calls = 0;
    const r = await runWithRetryOnError(() => { calls++; return cleanFail(); });
    expect(calls).toBe(1);
    expect(r.ok).toBe(false);
  });

  test("an attempt that keeps erroring/timing out stays failed after the single retry (no infinite retry)", async () => {
    let calls = 0;
    const r = await runWithRetryOnError(() => { calls++; return errored(); });
    expect(calls).toBe(2);
    expect(r.ok).toBe(false);
    expect(r.errored).toBe(true);
  });

  test("a first-attempt pass is never retried", async () => {
    let calls = 0;
    const r = await runWithRetryOnError(() => { calls++; return pass(); });
    expect(calls).toBe(1);
...
describe("ensureDeps (#12a: install is retried once on error/timeout via runWithRetryOnError)", () => {
  test("no package.json → ok, no install attempted (fine, no-gate repo)", async () => {
    ...
    try {
      ...
      expect(await ensureDeps(ws)).toEqual({ ok: true, detail: "no package.json" });
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });
...
describe("requiresBrowserEvidence = hasUiSurface && hasPlaywright (truth table)", () => {
  ...
  test("UI + Playwright → required", async () => {
  ...
  test("UI, no Playwright → not required", async () => {
  ...
  test("Playwright, no UI → not required", async () => {
    const ws = make({ ui: false, playwright: true });
    try { expect(requiresBrowserEvidence(ws)).toBe(false); } finally { rmSync(ws.dir, { recursive: true, force: true }); }
  });
  test("neither → not required", async () => {
