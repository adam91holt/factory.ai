...
## Security follow-ups (from metadata-block adversarial review, 2026-07-21)
The metadata-block review closed an RCE chain (untrusted block → repo override → clone attacker repo → postinstall RCE) at the source (start-anchor + stamping autonomous producers). Two defense-in-depth items remain, both deferred as config/ops changes rather than no-regress source fixes:
1. **`FACTORY_ALLOWED_REPOS` opt-in allowlist** enforced in `buildWorkspace` — a repo the factory will clone/build must be on the list. Closes the residual (a human deliberately placing an offset-0 block for an unintended repo). Opt-in / fail-open-on-empty so it never regresses existing tickets.
2. **`--ignore-scripts` on `ensureDeps`** (`bun install`/`npm ci`) — neuters postinstall RCE even if a bad repo is cloned. Judgment call: some repos need postinstall build steps, so verify per-repo before enabling (the build script usually re-runs them). Defense-in-depth on the RCE amplifier itself.
